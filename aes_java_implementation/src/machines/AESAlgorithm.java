...
public class AESAlgorithm {
	...
	// Ja tem de ter as subkeys definidas...
	public static byte[] encryptBloc(byte[] in, int pos, byte[][] keyExp,
			int keyLen) {
		Nb = 4;
		Nk = keyLen / 4;
		Nr = Nk + 6;
		...
		for (int i = 0; i < 16; i++) {
			state[i / 4][i % 4] = in[(i % 4 * 4 + i / 4) + pos];
		}

		state = AddRoundKey(state, keyExp, 0);
		for (int round = 1; round < Nr; round++) {
			state = SubBytes(state);
			state = ShiftRows(state);
			state = MixColumns(state);
			state = AddRoundKey(state, keyExp, round);
		}
		state = SubBytes(state);
		state = ShiftRows(state);
		state = AddRoundKey(state, keyExp, Nr);
	...
	public static byte[] decryptBloc(byte[] in, int pos, byte[][] keyExp,
			int keyLen) {
		Nb = 4;
		Nk = keyLen / 4;
		Nr = Nk + 6;
