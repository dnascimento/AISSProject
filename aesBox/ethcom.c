...
int EthCypherUDP(FILE* fp_r,char *file_name,int size, char mode)
{
    ...
    //WSADATA wsaData;
    int RecvSocket = INVALID_SOCKET,SendSocket = INVALID_SOCKET;
   ...
   // }
    ...
    iResult = bind(SendSocket, (struct sockaddr *) & RecvAddr, sizeof (RecvAddr));
    if (iResult != 0) {
	...
	while((data_len=fread(block_s+4,1,MAX_DATA+max_padding,fp_r)) !=0) 
	{
	 ...
	 do {
		...
		iResult = sendto(SendSocket,&block_s[total_bsent],data_len-total_bsent, 0, (struct sockaddr *) & SendAddr, sizeof (SendAddr));
		...
		while(brecv<bsent-4)  
		{
		 ...
		 iResult = recvfrom(SendSocket,block_r,sizeof(block_r), 0, (struct sockaddr *) & RecvAddr, &SenderAddrSize);
		 iResult = recvfrom(SendSocket,block_r,sizeof(block_r), 0, (struct sockaddr *) & RecvAddr, &SenderAddrSize);
    ...
    iResult = close(SendSocket);
    if (iResult == SOCKET_ERROR) {
        printf("closesocket failed ith error\n");
        //WSACleanup();
        return -1;
    }
